...
以下事项必须在 Step16 中以正式命令和记录重新确认，本文不把只读仓库快照当作长期有效结论：

- **Needs verification:** Step15 是否已合并到本地 `main`；
- **Needs verification:** 远端 `main` 是否已经更新；
- **Needs verification:** `main` 工作区是否干净，或所有脏变更是否可解释；
- **Needs verification:** `main` 上 build、typecheck 和 `check-executor codex` 是否重新通过。
